...
| --- | --- | --- |
| Lifecycle authority | Pi, OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode, Jcode | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
The hook reports Jcode lifecycle state and session identity to Herdr for authoritative `idle`, `working`, and `blocked` status and native restore. Jcode has no screen manifest fallback; state comes from the hook while Jcode runs inside a Herdr pane.

Herdr uses `~/.jcode` by default, or `JCODE_HOME` when set. The Jcode config directory must already exist. Install writes `hooks/herdr-agent-state.sh` and adds the Herdr command to each lifecycle event in the `[hooks]` table of `config.toml`. Jcode accepts several commands per event, so install appends Herdr's command and leaves existing hook commands in place; uninstall removes only Herdr's own entries and deletes the hook script.

Jcode swarm workers spawned into their own visible pane are skipped, because that pane already reports for itself.

Herdr resumes stored Jcode sessions with `jcode --resume <id>`.
