...
| --- | --- | --- |
| ライフサイクル権威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode、Jcode | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
このフックは Jcode のライフサイクル状態とセッション識別子を Herdr に報告し、`idle`、`working`、`blocked` の権威ある状態とネイティブな復元を提供します。Jcode には画面マニフェストのフォールバックがありません。状態は Herdr ペイン内で Jcode が動作している間、フックから取得されます。

Herdr は既定で `~/.jcode` を使用し、`JCODE_HOME` が設定されている場合はそちらを使用します。Jcode の設定ディレクトリが既に存在している必要があります。インストールは `hooks/herdr-agent-state.sh` を書き込み、`config.toml` の `[hooks]` テーブルの各ライフサイクルイベントに Herdr のコマンドを追加します。Jcode は 1 つのイベントに複数のコマンドを指定できるため、インストールは Herdr のコマンドを追記し、既存のフックコマンドはそのまま残します。アンインストールは Herdr 自身のエントリのみを削除し、フックスクリプトを削除します。

独自の可視ペインで起動された Jcode の swarm ワーカーは対象外です。そのペインは自分自身で報告するためです。

Herdr は保存された Jcode セッションを `jcode --resume <id>` で復元します。
