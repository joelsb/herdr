...
アイドルのペインも経過時間を持つため、まだ読んでいない完了結果と、すでに読んだ結果を区別できます。Herdr は 2 つの別々の時計を測定します。まだ見ていないペインは結果が現れた時点から、すでに見たペインは最後に見た時点から計測されるため、ペインを一目見るだけで «新しい» 状態に戻ります。

これにより 4 つのアイドル表示が生まれます。新しい未読の結果は **done**、未読のまましきい値を超えると **stale**、最近見たペインは **idle**、見たあとそのままにしたペインは **parked** です。未読の結果が stale になると、通常の注意が必要な通知も 1 回のアイドル期間につき 1 度だけ発生します。parked になる場合は、すでに自分で放置を選んでいるため通知はありません。

しきい値のデフォルトは 5 分で、設定画面の **idle aging** で変更するか、次のように設定できます:
