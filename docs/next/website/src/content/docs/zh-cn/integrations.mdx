...
| 集成类型 | 智能体 | 效果 |
| --- | --- | --- |
| 生命周期权威 | Pi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode、Jcode | 已安装且在为该窗格主动上报时,由钩子或插件事件决定 `idle`、`working` 和 `blocked`。对同一个生命周期权威,Herdr 不再使用屏幕清单兜底。 |
...
该钩子将 Jcode 的生命周期状态和会话标识报告给 Herdr，用于权威的 `idle`、`working` 和 `blocked` 状态以及原生恢复。Jcode 没有屏幕清单回退；当 Jcode 在 Herdr 窗格中运行时，状态来自该钩子。

Herdr 默认使用 `~/.jcode`，设置 `JCODE_HOME` 时则使用该目录。Jcode 配置目录必须已经存在。安装会写入 `hooks/herdr-agent-state.sh`，并在 `config.toml` 的 `[hooks]` 表中为每个生命周期事件添加 Herdr 命令。Jcode 允许每个事件配置多个命令，因此安装会追加 Herdr 的命令并保留已有的钩子命令；卸载只移除 Herdr 自己的条目并删除钩子脚本。

在自己的可见窗格中启动的 Jcode swarm 工作代理会被跳过，因为该窗格已经自行报告。

Herdr 使用 `jcode --resume <id>` 恢复已保存的 Jcode 会话。
