...
空闲的窗格也会计时，因此你尚未查看的完成结果与已经查看过的结果看起来并不相同。Herdr 会测量两个独立的时钟：你尚未查看的窗格从结果出现的时刻开始计时，而你已经查看过的窗格从上次查看的时刻开始计时，因此只要看一眼就能让它恢复为「新」状态。

由此产生四种空闲表现：新的未读结果为 **done**，未读结果超过阈值后为 **stale**，最近查看过的窗格为 **idle**，查看后就不再处理的窗格为 **parked**。未读结果变为 stale 时也会触发常规的需要关注通知，每个空闲周期只触发一次；变为 parked 时保持静默，因为那是你自己选择搁置的。

阈值默认为五分钟，可在设置的 **idle aging** 中修改，或进行配置：
