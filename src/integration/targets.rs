...
use super::config_edit::{
    ...
    remove_hermes_plugin_enabled, remove_hook_commands, remove_jcode_config_hooks,
...
/// Install the jcode reporter and register it for every lifecycle event.
///
/// jcode keeps hooks in `[hooks]` of its own `config.toml` and accepts either
/// one command or an array per event, so the config edit appends herdr's
/// command and preserves whatever the user already configured.
pub(crate) fn install_jcode() -> io::Result<JcodeInstallPaths> {
    ...
    let new_config = build_jcode_config_with_hooks(&existing_config, &hook_path);
    if new_config != existing_config {
        fs::write(&config_path, new_config)?;
    }
...
/// Remove the herdr reporter from jcode, leaving other hook commands intact.
pub(crate) fn uninstall_jcode() -> io::Result<JcodeUninstallResult> {
    let dir = jcode_dir()?;
    let hook_path = dir.join("hooks").join(JCODE_HOOK_INSTALL_NAME);
    let config_path = dir.join("config.toml");

    let updated_config = if config_path.is_file() {
        let existing_config = fs::read_to_string(&config_path)?;
        let new_config = remove_jcode_config_hooks(&existing_config, &hook_path);
        if new_config != existing_config {
            fs::write(&config_path, new_config)?;
            true
        } else {
